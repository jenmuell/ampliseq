    ...
    if ( params.input ) {
        ...
        ch_input_reads = Channel.fromSamplesheet("input") // meta: meta.sample, meta.run
            .map{ meta, readfw, readrv ->
                ...
                if ( !meta.single_end && !readrv ) { error("Entry `fastq_2` is missing in $params.input for $meta.sample, either correct the samplesheet or use `--single_end`, `--pacbio`, or `--iontorrent`") } // make sure that reverse reads are present when single_end isn't specified
                if ( !meta.single_end && ( readfw.getSimpleName() == meta.sample || readrv.getSimpleName() == meta.sample ) ) { error("Entry `sample` cannot be identical to simple name of `fastq_1` or `fastq_2`, please change `sample` in $params.input for sample $meta.sample") } // sample name and any file name without extensions aren't identical, because rename_raw_data_files.nf would forward 3 files (2 renamed +1 input) instead of 2 in that case
                if ( meta.single_end && ( readfw.getSimpleName() == meta.sample+"_1" || readfw.getSimpleName() == meta.sample+"_2" ) ) { error("Entry `sample`+ `_1` or `_2` cannot be identical to simple name of `fastq_1`, please change `sample` in $params.input for sample $meta.sample") } // sample name and file name without extensions aren't identical, because rename_raw_data_files.nf would forward 2 files (1 renamed +1 input) instead of 1 in that case
                return [meta, reads] }
